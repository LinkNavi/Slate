...
public:
    ...
    // Color property implementations
    QString colorKeyword() const { return m_colorKeyword; }
    QString colorType() const { return m_colorType; }
    QString colorString() const { return m_colorString; }
    QString colorComment() const { return m_colorComment; }
    QString colorNumber() const { return m_colorNumber; }
    QString colorFunction() const { return m_colorFunction; }
    QString colorPreprocessor() const { return m_colorPreprocessor; }
    QString colorOperator() const { return m_colorOperator; }

    void setColorKeyword(const QString &c) { m_colorKeyword = c; buildRules(); rehighlight(); emit colorsChanged(); }
    void setColorType(const QString &c) { m_colorType = c; buildRules(); rehighlight(); emit colorsChanged(); }
    void setColorString(const QString &c) { m_colorString = c; buildRules(); rehighlight(); emit colorsChanged(); }
    void setColorComment(const QString &c) { m_colorComment = c; buildRules(); rehighlight(); emit colorsChanged(); }
    void setColorNumber(const QString &c) { m_colorNumber = c; buildRules(); rehighlight(); emit colorsChanged(); }
    void setColorFunction(const QString &c) { m_colorFunction = c; buildRules(); rehighlight(); emit colorsChanged(); }
    void setColorPreprocessor(const QString &c) { m_colorPreprocessor = c; buildRules(); rehighlight(); emit colorsChanged(); }
    void setColorOperator(const QString &c) { m_colorOperator = c; buildRules(); rehighlight(); emit colorsChanged(); }
    ...
    Q_INVOKABLE void setSemanticTokens(const QVariantList &tokens) {
        m_semTokens = tokens;
        // Build a per-line lookup for fast access during highlighting
...
protected:
    void highlightBlock(const QString &text) override {
        // Step 1: Apply regex-based syntax rules as fallback
        ...
        // Step 2: Override with semantic tokens from LSP (higher fidelity)
        ...
        // Step 3: Diagnostic underlines (always on top)
        for (const auto &v : m_diagnostics) {
            ...
            if (blockNum >= startLine && blockNum <= endLine) {
                ...
                if (severity == 1)
                    diagFmt.setUnderlineColor(QColor("#E85454"));
                else if (severity == 2)
                    diagFmt.setUnderlineColor(QColor("#E8C44A"));
                else
                    diagFmt.setUnderlineColor(QColor("#4ECDC4"));
...
private:
    ...
    void addCppRules(const QTextCharFormat &kwFmt, const QTextCharFormat &typeFmt,
    ...
    void addPythonRules(const QTextCharFormat &kwFmt, const QTextCharFormat &typeFmt,
                        const QTextCharFormat &strFmt, const QTextCharFormat &numFmt,
        ...
        m_rules.append({QRegularExpression(R"('(?:[^'\\]|\\.)*')"), strFmt});
        m_rules.append({QRegularExpression(R"(#.*$)"), m_commentFormat});
        m_rules.append({QRegularExpression(R"(@\w+)"), typeFmt});  // Decorators
    }

    void addJsRules(const QTextCharFormat &kwFmt, const QTextCharFormat &typeFmt,
                    const QTextCharFormat &strFmt, const QTextCharFormat &numFmt,
                    const QTextCharFormat &fnFmt, const QTextCharFormat &,
                    const QTextCharFormat &) {
        m_rules.append({QRegularExpression(
        ...
        m_rules.append({QRegularExpression(R"("(?:[^"\\]|\\.)*")"), strFmt});
        m_rules.append({QRegularExpression(R"('(?:[^'\\]|\\.)*')"), strFmt});
        m_rules.append({QRegularExpression(R"(`(?:[^`\\]|\\.)*`)"), strFmt});  // Template literals
        m_rules.append({QRegularExpression(R"(//.*$)"), m_commentFormat});
        m_hasBlockComment = true;
        m_commentStart = "/*"; m_commentEnd = "*/";
    }

    void addRustRules(const QTextCharFormat &kwFmt, const QTextCharFormat &typeFmt,
                      const QTextCharFormat &strFmt, const QTextCharFormat &numFmt,
                      const QTextCharFormat &fnFmt, const QTextCharFormat &,
                      const QTextCharFormat &opFmt) {
        m_rules.append({QRegularExpression(
        ...
        m_rules.append({QRegularExpression(R"('[^'\\]')"), strFmt});
        m_rules.append({QRegularExpression(R"(//.*$)"), m_commentFormat});
        m_rules.append({QRegularExpression(R"(#\[.*?\])"), opFmt});  // Attributes
        m_rules.append({QRegularExpression(R"(\b[A-Z]\w*\b)"), typeFmt});  // PascalCase types
        m_hasBlockComment = true;
        m_commentStart = "/*"; m_commentEnd = "*/";
    }

    void addGoRules(const QTextCharFormat &kwFmt, const QTextCharFormat &typeFmt,
                    const QTextCharFormat &strFmt, const QTextCharFormat &numFmt,
                    const QTextCharFormat &fnFmt, const QTextCharFormat &,
                    const QTextCharFormat &) {
        m_rules.append({QRegularExpression(
    ...
    void addQmlRules(const QTextCharFormat &kwFmt, const QTextCharFormat &typeFmt,
                     const QTextCharFormat &strFmt, const QTextCharFormat &numFmt,
                     const QTextCharFormat &fnFmt, const QTextCharFormat &,
                     const QTextCharFormat &) {
        m_rules.append({QRegularExpression(
        ...
        m_rules.append({QRegularExpression(R"('(?:[^'\\]|\\.)*')"), strFmt});
        m_rules.append({QRegularExpression(R"(//.*$)"), m_commentFormat});
        m_rules.append({QRegularExpression(R"(\b\w+\s*:)"), typeFmt}); // Property bindings
        m_hasBlockComment = true;
        m_commentStart = "/*"; m_commentEnd = "*/";
    }

    // Map LSP semantic token type names to text formats using theme colors
    QTextCharFormat semanticFormat(const QString &typeName, int modifiers) {
        ...
        // Check modifiers
        // Bit 0: declaration, 1: definition, 2: readonly, 3: static, 4: deprecated
        if (modifiers & (1 << 0)) isBold = true;   // declaration
        if (modifiers & (1 << 1)) isBold = true;   // definition
        if (modifiers & (1 << 4)) {                 // deprecated
            fmt.setFontStrikeOut(true);
        }
        if (modifiers & (1 << 3)) isItalic = true;  // static

        if (typeName == "keyword" || typeName == "modifier") {
            fmt.setForeground(QColor(m_colorKeyword));
            isBold = true;
            ...
            // Keep default text color but maybe italic for params
            if (typeName == "parameter") isItalic = true;
            fmt.setForeground(QColor(m_colorType));  // slightly different from plain text
            ...
            fmt.setForeground(QColor(m_colorComment));
            isItalic = true;
            ...
            // Unknown type — don't override regex highlighting
