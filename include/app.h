...
// Editor must be defined before the using aliases that reference it.
struct Editor {
    EditorMode  mode        = EditorMode::NORMAL;
    std::string command_buf;
    std::string search_buf;        // typed query while in SEARCH mode
...
// ── Type aliases (require Editor to be complete) ──────────────────────────────
...
struct HighlightRule {
    std::string type_str;   // "keyword","string","comment","number","identifier"
    std::regex  compiled;
};

// ── Wren status segment ──────────────────────────────────────────────────────
struct WrenStatusSeg {
    WrenCallback cb;
};
